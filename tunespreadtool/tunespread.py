...
def calc_tune_spread(data, inputs, f_verbose):
    """calculates and returns the tune spread DeltaQ_x and DeltaQ_y due 
        to space charge with given optics parameters provided by
        the dictionaries / hash tables <i>inputs</i> and <i>data</i>."""
    r = rtimesm / inputs["mass"]
    beta = inputs["beta"]
    gamma = inputs["gamma"]
    sig_z = inputs["sig_z"]
    ...
    if f_verbose:
        stdout.write("\n\n")
    gaussprefactor = r * n_part / ( (2.0 * const.pi)**1.5 * beta**2 *
                                                    gamma**3 * sig_z )
    DeltaQ_x = lshape * gaussprefactor * integx
    DeltaQ_y = lshape * gaussprefactor * integy
